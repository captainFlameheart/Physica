...
namespace legacy::game_logic
{
	...
	)
	{
		...
		while (i < Vertex_Index_Count)
		{
			...
			::game_logic::util::proximity::insert_leaf_to_nonempty_tree(
	...
	::game_logic::cursor_types::initialize(environment);
	...
	{ // Fluid bounding box buffer
		...
		::game_logic::util::rigid_body::Triangle_Bounding_Box box
		{
			{
				0,
				0
	...
	{ // Position buffer
		...
		{
			GLuint const width{ 100u };
			::game_logic::util::rigid_body::Position position
			{
				{
	...
	{ // Velocity buffer
		...
		for (GLuint i = 0; i < environment.state.GPU_buffers.rigid_bodies.current_count; ++i)
		{
			::game_logic::util::rigid_body::Velocity velocity
			{
				{
	...
	::game_logic::util::proximity::initialize
	(
		environment.state.proximity_tree, game_logic_MAX_LEAF_COUNT(environment),
		MAX_CONTACT_COUNT(environment)
	);
	::game_logic::util::proximity::insert_leaf_to_empty_tree(
		environment.state.proximity_tree, game_logic_MAX_LEAF_COUNT(environment),
		0,
		0, 0, -1, -1
	);
...
void tick_positions(game_environment::Environment& environment)
{
	...
	if (::game_logic::util::proximity::is_empty(environment.state.proximity_tree))
	{

	}
	else if (::game_logic::util::proximity::has_single_node(environment.state.proximity_tree, game_logic_MAX_LEAF_COUNT(environment)))
	{
	...
	else
	{
		...
		for (GLuint i{ 0u }; i < changed_fluid_leaf_count; ++i)
		{
			...
			::game_logic::util::proximity::change_leaf_of_multinode_tree
			(
				environment.state.proximity_tree, game_logic_MAX_LEAF_COUNT(environment),
	...
	else
	{
		...
		{	// Fluid contact remove
				...
				void operator()(GLuint const contact_index, GLuint& next_contact_index)
				{
					if (contact_index < MAX_FLUID_CONTACT_COUNT(environment))
					{
						...
						if (contact_index != environment.state.GPU_buffers.fluid.contact_count.current_contact_count)
						{
							...
							::game_logic::util::proximity::move_contact
							(
			...
			::game_logic::util::proximity::remove_contacts
			(
				environment.state.proximity_tree, game_logic_MAX_LEAF_COUNT(environment),
		...
		{ // Fluid contact add
			...
			::game_logic::util::proximity::add_contacts
			(
				environment.state.proximity_tree, game_logic_MAX_LEAF_COUNT(environment),
				0u, changed_fluid_leaf_count,
	...
	case GLFW_MOUSE_BUTTON_LEFT:
		...
		case GLFW_PRESS:
			if (mods & GLFW_MOD_CONTROL)
			{
				...
				::game_logic::window_to_world::window_screen_cursor_position_to_world_position
				(
					environment,
			...
			if (!environment.state.point_grabbed)
			{
				...
				else
				{
					...
					else
					{
						...
						::game_logic::util::proximity::query_point_of_multinode_tree
						(
							environment.state.proximity_tree, game_logic_MAX_LEAF_COUNT(environment),
							cursor_world_x, cursor_world_y,
							on_leaf_found
...
)
{
	...
	::game_logic::window_to_camera::window_screen_cursor_position_to_camera_local_unit_z_vector
	(
		environment,
	...
	::game_logic::util::camera::local_world_position_to_world_position
	(
		environment,
		camera_local_world_cursor_x, camera_local_world_cursor_y,
	...
	::game_logic::util::camera::increase_camera_angle(environment, delta_angle);
	...
	::game_logic::util::camera::local_world_vector_to_world_vector
	(
		environment,
		camera_local_world_cursor_x, camera_local_world_cursor_y,
...
void tick(game_environment::Environment& environment)
{
	...
	GLint cursor_world_position[2u];
	::game_logic::window_to_world::window_screen_cursor_position_to_world_position(environment, &cursor_world_position[0], &cursor_world_position[1]);
	glClearNamedBufferSubData
	(
		environment.state.GPU_buffers.cursor.position.buffer,
		GL_RG32I,
...
void render(game_environment::Environment& environment)
{
	...
	if (environment.state.parent_bounding_boxes_visible && !::game_logic::util::proximity::is_empty(environment.state.proximity_tree))
	{
...
void free(game_environment::Environment& environment)
{
	...
	::game_logic::cursor_types::free(environment);
