...
{
	void query_vec4_data_layout(game_environment::Environment& environment, GLuint program)
	{
		::game_logic::shader_util::query::query_shader_storage_variable_offset_array_stride
		(
			program, "vec4_Data.data",
			environment.state.layouts.vec4_data.properties
		);
	}
}
