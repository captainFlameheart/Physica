...
{
	void query_vec2_data_layout(game_environment::Environment& environment, GLuint program)
	{
		::game_logic::shader_util::query::query_shader_storage_variable_offset_array_stride
		(
			program, "vec2_Data.data",
			environment.state.layouts.vec2_data.properties
		);
	}
}
