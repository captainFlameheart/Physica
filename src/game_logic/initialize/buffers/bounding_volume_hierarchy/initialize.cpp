...
namespace game_logic::initialize::buffers::bounding_volume_hierarchy
{
	void initialize(game_environment::Environment& environment)
	{
		glCreateBuffers(1u, &environment.state.buffers.bounding_volume_hierarchy.buffer);
		glNamedBufferStorage
		(
			environment.state.buffers.bounding_volume_hierarchy.buffer,
			environment.state.layouts.bounding_volume_hierarchy.block_state.buffer_data_size,
			nullptr,
		...
		(
			...
			(
				environment.state.buffers.bounding_volume_hierarchy.buffer,
				static_cast<GLintptr>(0), environment.state.layouts.bounding_volume_hierarchy.block_state.buffer_data_size,
				GL_MAP_READ_BIT | GL_MAP_PERSISTENT_BIT
			)
		);
