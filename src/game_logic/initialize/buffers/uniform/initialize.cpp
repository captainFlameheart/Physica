...
namespace game_logic::initialize::buffers::uniform
{
	void initialize(game_environment::Environment& environment)
	{
		...
		environment.state.buffers.uniform.mouse_button_event_offset = buffer_size;
		GLuint aligned_mouse_button_event_size{ binding_util::align_uniform_block_size(environment, environment.state.layouts.mouse_button_event.block_state.buffer_data_size) };
		GLuint full_aligned_mouse_button_event_size{ aligned_mouse_button_event_size * ::game_state::buffers::uniform::mouse_button_event_capacity };

		buffer_size += full_aligned_mouse_button_event_size;

		environment.state.buffers.uniform.scroll_event_offset = buffer_size;
		GLuint aligned_scroll_event_size{ binding_util::align_uniform_block_size(environment, environment.state.layouts.scroll_event.block_state.buffer_data_size) };
		GLuint full_aligned_scroll_event_size{ aligned_scroll_event_size * ::game_state::buffers::uniform::scroll_event_capacity };

		buffer_size += full_aligned_scroll_event_size;
		...
		environment.state.buffers.uniform.mouse_button_event_index = 0u;
		environment.state.buffers.uniform.scroll_event_index = 0u;
		...
		{
			environment.state.buffers.uniform.mouse_button_event_fences[i] = glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0u);
		}
		for (GLuint i{ 0u }; i < ::game_state::buffers::uniform::scroll_event_capacity; ++i)
		{
			environment.state.buffers.uniform.scroll_event_fences[i] = glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0u);
		}
