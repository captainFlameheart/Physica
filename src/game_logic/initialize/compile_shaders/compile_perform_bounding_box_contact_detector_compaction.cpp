...
namespace game_logic::initialize::compile_shaders
{
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/pre_constraint_spawners/perform_compaction/perform_bounding_box_contact_detector_compaction.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::pre_constraint_spawners::perform_compaction::Indices::perform_bounding_box_contact_detector_compaction)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
