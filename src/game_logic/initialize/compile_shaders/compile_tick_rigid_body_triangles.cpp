...
{
	void compile_tick_rigid_body_triangles
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/bounding_volume_hierarchy/tick_rigid_body_triangles.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::bounding_volume_hierarchy::leafs::Indices::rigid_body_triangles)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
