...
{
	void compile_draw_source_image_layer
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("draw/holographic_radiance_cascades/source_layer/draw.vert")
		);
		...
		(
			...
			compile_environment.readonly_prefix_source,
			::util::shader::file_to_string("draw/holographic_radiance_cascades/source_layer/draw.frag")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::draw::holographic_radiance_cascades::Indices::source_layer)] = ::util::shader::create_program
		(
