...
{
	void compile_find_inner_bounding_box_height_changes
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/bounding_volume_hierarchy/find_height_changes.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::bounding_volume_hierarchy::initialize_inner_bounding_box_traversal::Indices::find_height_changes)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
