...
{
	void compile_set_inner_bounding_box_tick_commands
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/bounding_volume_hierarchy/set_commands.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::bounding_volume_hierarchy::initialize_inner_bounding_box_traversal::Indices::set_commands)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
