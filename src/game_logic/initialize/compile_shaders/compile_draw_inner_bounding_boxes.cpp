...
{
	void compile_draw_inner_bounding_boxes
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("draw/bounding_volume_hierarchy/inner_bounding_boxes/default/draw.vert")
		);
		...
		(
			...
			compile_environment.readonly_prefix_source,
			::util::shader::file_to_string("draw/bounding_volume_hierarchy/inner_bounding_boxes/default/draw.frag")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::draw::entities::bounding_volume_hierarchy::Indices::inner_bounding_boxes)] = ::util::shader::create_program
		(
