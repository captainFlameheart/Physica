...
namespace game_logic::initialize::compile_shaders
{
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("draw/cursor/default/draw.vert")
		);
		...
		(
			...
			compile_environment.readonly_prefix_source,
			::util::shader::file_to_string("draw/cursor/default/draw.frag")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::draw::cursor::Indices::cursor)] = ::util::shader::create_program
		(
