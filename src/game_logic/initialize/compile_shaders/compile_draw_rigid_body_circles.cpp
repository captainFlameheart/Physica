...
{
	void compile_draw_rigid_body_circles
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("draw/bodies/rigid_bodies/circles/default/draw.vert")
		);
		...
		(
			...
			compile_environment.readonly_prefix_source,
			::util::shader::file_to_string("draw/bodies/rigid_bodies/circles/default/draw.frag")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::draw::entities::bodies::Indices::rigid_body_circles)] = ::util::shader::create_program
		(
