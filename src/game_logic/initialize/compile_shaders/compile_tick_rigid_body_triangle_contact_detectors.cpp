...
{
	void compile_tick_rigid_body_triangle_contact_detectors
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/constraint_spawners/tick_rigid_body_triangle_contact_detectors.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::constraint_spawners::Indices::rigid_body_triangle_contact_detectors)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
