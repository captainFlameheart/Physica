...
{
	void compile_tick_rigid_body_circle_contact_constraints
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/constraints/tick_rigid_body_circle_contact_constraints.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::constraints::Indices::rigid_body_circle_contact_constraints)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
