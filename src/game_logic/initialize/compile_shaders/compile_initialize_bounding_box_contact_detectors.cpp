...
{
	void compile_initialize_bounding_box_contact_detectors
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("reusable/initialize_contacts/initialize_bounding_box_contact_detectors.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::reusable::initialize_contacts::Indices::initialize_bounding_box_contact_detectors)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
