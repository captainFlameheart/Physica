...
{
	void compile_commit_constraint_spawner_counts
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/pre_constraint_spawners/commit_counts/commit_counts.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::pre_constraint_spawners::commit_counts::Indices::commit_counts)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
