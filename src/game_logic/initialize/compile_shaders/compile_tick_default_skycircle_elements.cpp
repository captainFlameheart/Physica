...
{
	void compile_tick_default_skycircle_elements
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/skycircle_elements/tick_default_skycircle_elements.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::skycircle_elements::Indices::default_elements)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
