...
{
	void compile_reset_bounding_box_contact_detector_deaths
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("reusable/initialize_contacts/reset_bounding_box_contact_detector_deaths.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::reusable::initialize_contacts::Indices::reset_bounding_box_contact_detector_deaths)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
