...
{
	void compile_on_update_tick_counts
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/on_update_counts.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::update_counts::Indices::on_update_counts)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
