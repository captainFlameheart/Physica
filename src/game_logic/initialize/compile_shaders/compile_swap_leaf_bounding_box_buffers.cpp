...
{
	void compile_swap_leaf_bounding_box_buffers
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/bounding_volume_hierarchy/swap_leaf_bounding_box_buffers.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::bounding_volume_hierarchy::swap_leaf_bounding_box_buffers::Indices::swap_leaf_bounding_box_buffers)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
