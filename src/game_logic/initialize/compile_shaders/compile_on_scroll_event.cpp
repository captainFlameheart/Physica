...
namespace game_logic::initialize::compile_shaders
{
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("on_scroll_event/on_scroll_event.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::on_scroll_event::Indices::on_scroll_event)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
