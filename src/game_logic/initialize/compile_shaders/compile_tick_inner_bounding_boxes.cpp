...
{
	void compile_tick_inner_bounding_boxes
	(
	...
	)
	{
		...
		(
			...
			::util::shader::file_to_string("tick/bounding_volume_hierarchy/tick_inner_bounding_boxes.comp")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::tick::process_entities::bounding_volume_hierarchy::inner_bounding_boxes::Indices::inner_bounding_boxes)] = ::util::shader::create_program
		(
			compile_environment.shader_group.compute_shader
		);
	}
