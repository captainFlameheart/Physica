...
{
	void compile_draw_timing_deadline
	(
	...
	)
	{
		...
		(
			...
			blocks,
			::util::shader::file_to_string("draw/profiling/timing/deadline/draw.vert")
		);
		...
		(
			...
			compile_environment.readonly_prefix_source,
			blocks,
			::util::shader::file_to_string("draw/profiling/timing/deadline/draw.frag")
		);

		environment.state.shaders[static_cast<GLuint>(::game_state::shader_indices::draw::profiling::Indices::deadline)] = ::util::shader::create_program
		(
