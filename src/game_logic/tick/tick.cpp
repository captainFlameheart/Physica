...
namespace game_logic::tick
{
	...
	void clear_detector_deaths(game_environment::Environment& environment)
	{
	...
	void perform_detector_compaction(game_environment::Environment& environment)
	{
	...
	void reset_bounding_box_contact_detectors_if_needed(game_environment::Environment& environment)
	{
		{
	...
	void commit_constraint_counts(game_environment::Environment& environment)
	{
		...
		{
			(constraint_type_count + commit_constraint_counts_local_size - 1u) / commit_constraint_counts_local_size
		};
		glDispatchCompute(commit_constraint_counts_work_group_count, 1u, 1u);
	}

	void clear_constraint_deaths(game_environment::Environment& environment)
	{
	...
	void perform_constraint_compaction(game_environment::Environment& environment)
	{
	...
	void tick_constraints(game_environment::Environment& environment)
	{
	...
	void tick(game_environment::Environment& environment)
	{
		if (environment.state.tick_count % 1u == 0u)	// TODO: Make into proper feature
		{
			...
			clear_detector_deaths(environment);	// Can these steps be put next to something else?
			...
			commit_constraint_counts(environment);

			glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT | GL_COMMAND_BARRIER_BIT);

			clear_constraint_deaths(environment);
