		...
		while (!glfwWindowShouldClose(window))
		{
			...
			if (draw_count % 120u == 0u)
			{
				...
				if (time_elapsed_query_done == GL_TRUE)
				{
					...
					std::cout << "Triangle contact count: " << game_environment.state.current_triangle_contact_count << ", Time elapsed : " << time_elapsed << " ns = " << time_elapsed_double * 1e-6 << " ms = " << time_elapsed_double * 6e-6 << " % of draw frame = " << time_elapsed_double * 1.2e-5 << " % of physics step" << std::endl;
				}
			}
			++draw_count;
