...
void main()
{
	if (gl_VertexID.x == 0u)
	{
		uint count = distance_constraints.count;
		uint body = distance_constraints.distance_constraints[count].bodies.x;
		vec2 local_point = distance_constraints.distance_constraints[count].local_points[0u];
